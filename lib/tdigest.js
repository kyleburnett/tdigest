...
TDigest.prototype.deserialize = function(obj) {
    this.discrete = obj[0];
    this.delta = obj[1];
    this.K = obj[2];
    this.CX = obj[3];
    this.nreset = obj[4];
    this.n = obj[5];
    this.last_cumulate = obj[6];
    this.centroids.deserialize(obj[7], node_deserializer);
};

TDigest.prototype.serialize = function() {
    return [
        this.discrete, this.delta, this.K, this.CX, this.nreset, this.n, this.last_cumulate,
        this.centroids.serialize(node_serializer)
    ];
};
...
function node_deserializer(arr) {
    if (arr.length === 3) {
        return {
            mean: arr[0],
            n: arr[1],
...
function node_serializer(node) {
    if (typeof node.mean_cumn === 'undefined') {
        return [node.mean, node.n, node.cumn];
    } else {
        return [node.mean, node.n, node.cumn, node.mean_cumn];
    }
}
